...
local function teleportToLobby()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		...
		if player.Team ~= nil then player.Team = nil end
...
local function teleportToArena(players)
	...
	if CurrentMap then
		for _, child in ipairs(CurrentMap:GetDescendants()) do
			if child:IsA("SpawnLocation") then table.insert(spawns, child) end
		end
	end
	...
	if gameMode == "TDM" then
		for i, player in ipairs(players) do
			if i % 2 == 1 then player.Team = Teams.RedTeam else player.Team = Teams.BlueTeam end
		end
	end
...
local function loadMap(mapName)
	...
	if not mapTemplate then return end
...
local function resetScores()
	for _, player in ipairs(Players:GetPlayers()) do
		...
		if stats then stats.Kills.Value = 0 end
	end
end
...
local function startRound(mode, participants)
	...
	if gameMode == "FFA" then broadcast("MODE: FREE FOR ALL", Color3.new(1, 0.4, 0.4))
	elseif gameMode == "TDM" then broadcast("MODE: TEAM DEATHMATCH", Color3.new(0.4, 0.6, 1))
	elseif gameMode == "BUILD" then broadcast("MODE: BUILD BATTLE", Color3.new(1, 0.9, 0.2)) end
	task.wait(3)
	...
	if targetMap.type == "Community" then
		broadcast("MAP: STAGE " .. targetMap.stageId .. "\nBY " .. string.upper(targetMap.creator), Color3.new(1, 0.8, 0.2))
		task.wait(3)
		loadMap("Map_BuildBase")

		local getStageByIdBindable = ReplicatedStorage:FindFirstChild("GetCommunityStageById")
		local stageInfo = nil
		if getStageByIdBindable then stageInfo = getStageByIdBindable:Invoke(targetMap.stageId) end

		if stageInfo and stageInfo.data then
			local BLOCK_SIZE = 2
			for _, data in ipairs(stageInfo.data) do
				local block
				if data.shape == "Wedge" then block = Instance.new("WedgePart")
				else block = Instance.new("Part") block.Shape = Enum.PartType.Block end
				block.Name = "PlayerWall"
				block.Size = Vector3.new(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE)
				block.Anchored = true
				if data.color then block.Color = Color3.new(data.color[1], data.color[2], data.color[3]) else block.Color = Color3.fromRGB(0, 200, 255) end
				if data.material then
					local s, mat = pcall(function() return Enum.Material[data.material] end)
				...
				block.CFrame = CFrame.new(data.cx, data.cy, data.cz, data.r00, data.r01, data.r02, data.r10, data.r11, data.r12, data.r20, data.r21, data.r22)
				if CurrentMap then block.Parent = CurrentMap else block.Parent = workspace end
			end
		end
	else
...
local function onHumanoidDied(humanoid, player)
	...
	if creatorTag and creatorTag.Value then
		...
		if killer:IsA("Player") then killerChar = killer.Character
		elseif killer:IsA("Model") then killerChar = killer end
		
		if killerChar and player then cameraEvent:FireClient(player, "Kill", killerChar) end
		
		if killer:IsA("Player") and killer ~= player then
			-- チーム戦での同士討ちチェック
			if gameMode == "TDM" and player and killer.Team == player.Team and player.Team ~= nil then return end
...
local function gameLoop()
	while true do
		...
		if readyCount == 0 then
			if SHOW_LOBBY_MESSAGE then broadcast("Stand in a ZONE to join!", Color3.new(0.5, 1, 1)) else broadcast("", Color3.new(1, 1, 1)) end
			task.wait(0.5)
		else
			...
			for i = INTERMISSION_TIME, 1, -1 do
				...
				if countdownCancelled then break end
			end
